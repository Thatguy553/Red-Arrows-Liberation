    ...
    if ([5] call KPLIB_fnc_hasPermission) then {
        ...
        _near_intel = (getPosATL player) nearEntities [KPLIB_intelObjectClasses, 5];
        {
            if ((captive _x) && !(_x in _actionned_captive_units) && !((side group _x) == GRLIB_side_friendly) && !(_x getVariable ["ACE_isUnconscious", false])) then {
                _x addAction ["<t color='#FFFF00'>" + localize "STR_SECONDARY_CAPTURE" + "</t>",{[_this select 0] join (group player);},"",-850,true,true,"","(isNull objectParent player) && (side group _target != GRLIB_side_friendly) && (captive _target)"];
...

        {
            if !(_x in _actionned_intel_items) then {
                _x addAction ["<t color='#FFFF00'>" + localize "STR_INTEL" + "</t>",{[_this select 0] remoteExecCall ["intel_remote_call", 2];},"",-849,true,true,"","(isNull objectParent player)"];
